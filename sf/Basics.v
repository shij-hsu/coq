...
(* FILL IN HERE *)
Theorem negeation_fn_applied_twice:
  forall (f:bool->bool),(forall (x:bool),f x=negb x)->
                        forall (b:bool), f (f b)=b.
Proof.
  intros. simpl. repeat rewrite H. rewrite negb_involutive. reflexivity. Qed.
(** [] *)
...
Fixpoint bin_to_nat (b:bin):nat:=
  ...
  |One_Twice b'=>2*(bin_to_nat b')+1
  end.
(*notice binary number is inverted order to read 'Twice' for 0,
'One_Twice for 1, therefore 0->Twice Zero, 1->One_Twice Zero,
2->10->Twice One_Twice Zero, etc.
*)
Eval vm_compute in bin_to_nat (Zero).
Eval vm_compute in bin_to_nat (One_Twice Zero).
Eval vm_compute in bin_to_nat (Twice (One_Twice Zero)).
Eval vm_compute in bin_to_nat (One_Twice (One_Twice Zero)).
Eval vm_compute in bin_to_nat (Twice (Twice (One_Twice Zero))).
(** [] *)
