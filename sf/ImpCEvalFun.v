...
Definition pup_to_n : com :=
  (Y::=ANum 0;;WHILE (BLe (ANum 1) (AId X)) DO Y::=APlus (AId Y) (AId X);;X::=AMinus (AId X) (ANum 1) END).
...
(** Write a [While] program that sets [Z] to [0] if [X] is even and
    ...
    program. *)
Definition check_even :=
  (Z::=ANum 0;;WHILE BLe (ANum 1) (AId X) DO Z::=AMinus (ANum 1) (AId Z);;X::=AMinus (AId X) (ANum 1) END).
(* FILL IN HERE *)
Example check_even_1:
  test_ceval (t_update empty_state X 10) check_even=Some (0,0,0).
Proof. reflexivity. Qed.
(** [] *)
...
Proof.
  ...
  - destruct IHHce1. destruct IHHce2. exists (S (x+x0)). simpl.
    assert (ceval_step st c1 (x + x0)=Some st').
    { apply (ceval_step_more x (x+x0)). omega. apply H. }
    rewrite H1. apply (ceval_step_more x0 (x+x0)). omega. apply H0.
  - destruct IHHce. exists (S x). simpl. rewrite H. apply H0.
  - destruct IHHce. exists (S x). simpl. rewrite H. apply H0.
  - exists (S O). simpl. rewrite H. reflexivity.
  - destruct IHHce1. destruct IHHce2. exists (S (x+x0)). simpl.
    rewrite H. assert (ceval_step st c (x + x0)=Some st').
    { apply (ceval_step_more x (x+x0)). omega. apply H0. }
    rewrite H2. apply (ceval_step_more x0 (x+x0)). omega. apply H1. Qed.
  (* FILL IN HERE *) 
(** [] *)
