...
Proof. intros. destruct p as [n m]. simpl. reflexivity. Qed.
(** [] *)
...
Fixpoint nonzeros (l:natlist) : natlist:=
  match l with
  |nil=>nil
  |x::l'=>(if beq_nat x 0 then nonzeros l' else x::(nonzeros l'))
  end.
...
Fixpoint oddmembers (l:natlist) : natlist:=
  match l with
  |nil=>nil
  |x::l'=>(if evenb x then oddmembers l' else x::(oddmembers l'))
  end.
...
Fixpoint countoddmembers (l:natlist) : nat:=
  match l with
  |nil=>0
  |x::l'=>if evenb x then countoddmembers l' else 1+(countoddmembers l')
  end.
...
Fixpoint count (v:nat) (s:bag) : nat:=
  match s with
  |nil=>0
  |x::s'=>if (beq_nat v x) then 1+(count v s') else count v s'
  end.

Ltac tac1:=simpl; reflexivity.
...
Fixpoint remove_one (v:nat) (s:bag) : bag :=
  match s with
  |nil=>s
  |x::l'=>if beq_nat v x then l' else x::(remove_one v l')
  end.
...
Fixpoint remove_all (v:nat) (s:bag) : bag :=
  match s with
  |nil=>s
  |x::s'=>if beq_nat v x then remove_all v s' else x::remove_all v s'
  end.
...
Fixpoint subset (s1:bag) (s2:bag) : bool :=
  match s1 with
  |nil=>true
  |x::s1'=>andb (leb (count x s1) (count x s2)) (subset s1' s2)
  end.
...
Theorem bag_theorem : forall (x:nat) (s:bag), count x (add x s)=1+count x s.
Proof. intros. induction s. simpl. rewrite<-beq_nat_refl. reflexivity.
...
Theorem app_nil_r : forall l : natlist,
  l ++ [] = l.
Proof. intros. induction l. simpl. reflexivity. simpl. rewrite IHl.
       reflexivity. Qed.
...
Proof. intros. induction l. simpl. reflexivity.
       simpl. assert (forall (l1 l2:natlist), rev (l1++l2)=rev l2++(rev l1)).
       { intros. induction l1. simpl. rewrite app_nil_r. reflexivity.
         simpl. rewrite IHl1. rewrite app_assoc. reflexivity. }
       simpl. rewrite H. simpl. rewrite IHl. reflexivity. Qed.
...
Proof. intros. rewrite app_assoc. rewrite app_assoc. reflexivity. Qed.

(** An exercise about your implementation of [nonzeros]: *)

Lemma nonzeros_app : forall l1 l2 : natlist,
  nonzeros (l1 ++ l2) = (nonzeros l1) ++ (nonzeros l2).
Proof. intros. induction l1. simpl. reflexivity. simpl.
       case n. simpl. rewrite IHl1. reflexivity.
       case n. simpl. rewrite IHl1. intros. reflexivity.
       intros. simpl. rewrite IHl1. reflexivity. Qed.
(** [] *)
...
Fixpoint beq_natlist (l1 l2 : natlist) : bool:=
  ...
  |x::l1'=>
   match l2 with
   |nil=>false
   |y::l2'=>if beq_nat x y then beq_natlist l1' l2'
            else false
   end
  end.
...
Proof. intros. induction s. simpl. reflexivity.
       case n. simpl.
       assert (H1:forall n:nat,leb n (S n)=true).
       { intros. induction n0. simpl. reflexivity. simpl.
...
(* FILL IN HERE *)
Theorem bag_count_sum:forall (s1 s2:bag), count 0 s1+(count 0 s2)=count 0 (sum s1 s2).
Admitted.
(** [] *)
...
Proof. intros. rewrite<-rev_involutive. rewrite<-H. rewrite rev_involutive. reflexivity. Qed.
(* FILL IN HERE *)
(** [] *)
...
Proof. intros. induction x. simpl. rewrite<-beq_nat_refl. reflexivity. Qed.
(** [] *)
...
Proof. intros. induction k. simpl. rewrite<-beq_nat_refl. reflexivity. Qed.
(** [] *)
...
Proof. intros. induction d. simpl. rewrite H. reflexivity.
