...
Proof.
  ...
  - inversion H. subst.
    assert (t_update (t_update st X (aeval st a)) X m=st).
    { destruct H0. unfold t_update. apply functional_extensionality. intros. destruct (beq_id X x) eqn:E.
      rewrite <-H1. assert (X=x). apply beq_id_true_iff.
      apply E. rewrite H2. reflexivity. reflexivity. }
    rewrite H1. destruct H0. apply H0.
  - inversion H. subst. destruct H0.
    assert (t_update (t_update st X (aeval st a)) X m=st).
    { unfold t_update. apply functional_extensionality. intros. destruct (beq_id X x) eqn:E.
      rewrite <-H1. assert (X=x). apply beq_id_true_iff.
      apply E. rewrite H2. reflexivity. reflexivity. }
    rewrite H2. unfold t_update. reflexivity. Qed.
  (* FILL IN HERE *)
(** [] *)
...
Proof.
  ...
  exists (st X). assert (t_update (t_update st X (aeval st a)) X (st X)=st).
  { unfold t_update. apply functional_extensionality.
    intros. destruct (beq_id X x) eqn:E.
    assert (X=x). apply beq_id_true_iff. apply E.
    rewrite H1. reflexivity. reflexivity. }
  rewrite H1. split. apply H0. unfold t_update.
...
(** Translate these informal Hoare triples...
   ...
   to prove them. *)
Lemma hoare_sub_ex1':
  {{fun st=>(st X+1<=5)}}X::=APlus (AId X) (ANum 1)
                       {{fun st=>(st X<=5)}}.
Proof. eapply hoare_consequence_pre.
   apply hoare_asgn. unfold assert_implies, assn_sub, t_update. intros. simpl. apply H. Qed.
(* FILL IN HERE *)
(** [] *)
...
Proof. eapply hoare_seq. apply hoare_asgn.
   unfold assn_sub, t_update, hoare_triple. simpl.
   intros. split.
   -inversion H. unfold t_update. simpl. subst. auto.
   - reflexivity. Qed.
  (* FILL IN HERE *)
(** [] *)
...
Theorem swap_exercise :
  ...
  swap_program
  {{fun st => st Y <= st X}}.
Proof. unfold swap_program. eapply hoare_seq.
   eapply hoare_seq. apply hoare_asgn. apply hoare_asgn.
   unfold hoare_triple, assn_sub, t_update.
   intros. simpl. inversion H. subst. unfold t_update.
   simpl. apply H0. Qed.
  (* FILL IN HERE *)
(** [] *)
...
(* FILL IN HERE *)
(* 第一条赋值语句可能改变状态st为st'，使得aeval st' a<>n *)
(** [] *)
...
Proof. eapply hoare_if.
   - unfold bassn. eapply hoare_consequence_pre.
     apply hoare_asgn. unfold assert_implies, assn_sub, t_update. simpl. intros. destruct H.
     assert (forall a b, leb a b=true->b=a+(b-a)).
     { induction a. simpl. intros. omega.
       intros. destruct b. simpl. inversion H1.
       simpl in *. pattern b at 1. rewrite (IHa b H1).
       reflexivity. }
     apply H1. assumption.
   - unfold bassn. eapply hoare_consequence_pre.
...
Lemma hoare_if1_good :
  ...
  {{ fun st => st X = st Z }}.
Proof. (* FILL IN HERE *)
  unfold hoare_triple. intros. inversion H. Qed.
End If1.
(** [] *)
...
Inductive ceval : state -> com -> state -> Prop :=
  ...
  | E_WhileLoop : forall st st' st'' b1 c1,
      ...
      ceval st' (WHILE b1 DO c1 END) st'' ->
      ceval st (WHILE b1 DO c1 END) st''
  | E_RepeatOnce: forall st st' c b,
      ceval st c st'->beval st' b=true->
      ceval st (REPEAT c UNTIL b END) st'
  | E_RepeatLoop:forall st st' st'' c b,
      ceval st c st'->ceval st' (REPEAT c UNTIL b END) st''->
      ceval st (REPEAT c UNTIL b END) st''
(* FILL IN HERE *)
.
...
Proof. unfold ex1_repeat. apply E_RepeatOnce.
   apply E_Seq with (t_update empty_state X 1).
   apply E_Ass. auto. eapply E_Ass. auto.
   unfold beval. auto. Qed.
  (* FILL IN HERE *) 
...
Definition havoc_pre (X : id) (Q : Assertion) : Assertion :=
  fun st=>(forall a, Q (t_update st X a)).
  (* REPLACE THIS LINE WITH   := _your_definition_ . *)
Theorem hoare_havoc : forall (Q : Assertion) (X : id),
  {{ havoc_pre X Q }} HAVOC X {{ Q }}.
Proof. intros. unfold havoc_pre. unfold hoare_triple.
   intros. inversion H. subst. apply H0. Qed.
  (* FILL IN HERE *)
