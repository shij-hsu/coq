...
Definition equiv_classes : list (list com) :=
  [[prog_a;prog_d];[prog_b];[prog_c];[prog_e];[prog_f;prog_g;prog_h];[prog_i]].
...
Proof. intros. unfold cequiv. intros. split.
   ...
   - intros. apply E_Seq with st'. apply H. apply E_Skip. Qed.
(* FILL IN HERE *)
(** [] *)
...
Theorem IFB_false: forall b c1 c2,
  ...
  cequiv
    (IFB b THEN c1 ELSE c2 FI)
    c2.
Proof. intros. unfold bequiv in *. unfold cequiv. intros. split.
   - intros. simpl in *. inversion H0. subst. rewrite (H st) in H6. inversion H6. subst. apply H7.
   - intros. simpl in *. apply E_IfFalse. exact (H st). apply H0. Qed.
(* FILL IN HERE *)
(** [] *)
...
Proof. intros. unfold cequiv. intros. split.
   - intros. inversion H. subst. apply E_IfFalse.
     ...
     subst. apply E_IfTrue. simpl. rewrite H5. reflexivity.
     apply H6.
   - intros. inversion H. subst. apply E_IfFalse.
     simpl in *. assert (beval st b = negb true).
     { apply negb_sym. symmetry. apply H5. }
     simpl in *. apply H0. apply H6.
     subst. inversion H. subst. apply E_IfTrue.
     rewrite H7 in H5. inversion H5. apply H6. subst.
     apply E_IfTrue. simpl in *. assert (beval st b= negb false).
     { apply negb_sym. symmetry. apply H5. }
     simpl in *. apply H0. apply H8. Qed.
  (* FILL IN HERE *)
(** [] *)
...
Proof. intros. unfold cequiv. intros. split.
   - intros. destruct (WHILE_true_nonterm b c st st').
     apply H. apply H0.
   - intros. assert False. apply (WHILE_true_nonterm BTrue SKIP st st').  unfold bequiv. simpl. intros. reflexivity.
     apply H0. inversion H1. Qed.
  (* FILL IN HERE *)
(** [] *)
