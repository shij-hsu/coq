...
Theorem plus_swap : forall n m p : nat,
  n + (m + p) = m + (n + p).
Proof.
  (*assert (forall (n m p:nat),n+(m+p)=(n+m)+p).*)
...
Theorem mult_comm : forall m n : nat,
  m * n = n * m.
Proof.
  assert (mult_distr:forall n m p:nat,p*(n+m)=p*n+p*m).
  intros. induction p. auto. simpl. rewrite IHp.
  rewrite plus_assoc. rewrite plus_swap.
  rewrite plus_assoc.
  pattern (m+(n+p*n)) at 1.
  rewrite plus_swap. rewrite plus_assoc. reflexivity.
  intros. induction n as [|n' IHn']. auto. simpl.
  assert (forall (x y:nat),x*S y=x+x*y).
  { intros. induction x. auto. simpl. rewrite IHx. rewrite plus_swap.
    reflexivity. }
  rewrite H. rewrite IHn'. reflexivity. Qed.
(** [] *)
...
Theorem andb_false_r : forall b : bool,
  andb b false = false.
Proof. intros. destruct b as [].
       simpl. reflexivity. simpl. reflexivity. Qed.

Theorem plus_ble_compat_l : forall n m p : nat,
  leb n m = true -> leb (p + n) (p + m) = true.
Proof.
  intros. induction p. simpl. rewrite H. reflexivity.
...
Proof. intros. induction n. simpl. reflexivity. simpl.
       assert(Hx:forall x:nat,x+0=x).
       { intros. induction x. auto. simpl. rewrite IHx. auto. }
       rewrite Hx. reflexivity. Qed.
...
Proof. intros. induction p.
       assert (Hx:forall x:nat,x*0=0).
       {induction x. simpl. reflexivity. simpl. rewrite IHx. reflexivity. }
       repeat rewrite Hx. auto.
       assert (Hd:forall x y:nat,x*S y=x+x*y).
       { intros. induction x. simpl. reflexivity.
         simpl. rewrite IHx. rewrite plus_swap. reflexivity. }
       repeat rewrite Hd. rewrite plus_assoc. rewrite IHp.
       rewrite plus_assoc.
       pattern (n+m+n*p) at 1. rewrite<-plus_assoc.
       pattern (m+n*p) at 1. rewrite plus_comm.
       pattern (n+(n*p+m)) at 1. rewrite plus_assoc.
       reflexivity. Qed.
...
Theorem plus_swap' : forall n m p : nat,
  n + (m + p) = m + (n + p).
Proof. intros. rewrite plus_assoc.
...
(* FILL IN HERE *)
Theorem bin_to_nat_pres_incr:forall b:bin,bin_to_nat (incr b)=
                                          S (bin_to_nat b).
Proof.
  intros. induction b. simpl. reflexivity.
  simpl. rewrite<-plus_n_Sm.
  assert (Hx:forall x:nat,x+0=x).
  { intros. induction x. reflexivity. simpl. rewrite IHx. reflexivity. }
  rewrite Hx. reflexivity. simpl. rewrite IHb.
  simpl.
  assert (Hx:forall x:nat, S (x+0)=(x+0)+1).
  { intros. induction x. simpl. reflexivity. simpl. rewrite IHx. reflexivity. }
  rewrite Hx. rewrite plus_assoc. reflexivity. Qed.
(** [] *)
...
Theorem nat_inv:forall n:nat, bin_to_nat (nat_to_bin n)=n.
Proof.
  intros. induction n. simpl. reflexivity. simpl.
  rewrite bin_to_nat_pres_incr. rewrite IHn. reflexivity. Qed.

Theorem bin_inv:forall b:bin, nat_to_bin (bin_to_nat b)=b.
Proof. intros. induction b. simpl. reflexivity.
Admitted.
(*Explain: Becase the natural number 0 can be written as Zero, Twice Zero, Twice (Twice Zero)... in binary number, so bin_to_nat of then are same therefore nat_to_bin (bin_to_nat ) of them is same, but Zero~=Twice Zero as we know.*)
(** [] *)

Definition normalize b:bin:=nat_to_bin (bin_to_nat b).
(* ################################################################# *)
