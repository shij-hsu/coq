...
Proof.
  (* FILL IN HERE *)
  simpl. eapply ST_Plus2. eapply ST_Plus2. eapply ST_PlusConstConst. Qed.
(** [] *)
...
Theorem step_deterministic :
  deterministic step.
Proof. unfold deterministic. intros. generalize dependent y1.
   induction H0.
   - intros. inversion H.
     + reflexivity.
     + inversion H3.
   ...
   - intros. inversion H.
     ...
     + inversion H3. rewrite <-H6 in H0. inversion H0.
   - intros. inversion H. inversion H1.
     + rewrite <-H5 in H0. inversion H0.
     + rewrite <-H2 in H6. inversion H6.
     + rewrite <-(IHstep t2'0 H7). rewrite <-H2. reflexivity. Qed.
  (* FILL IN HERE *)
(** [] *)
...
Proof. exists (P (C 0) (C 0)). split. apply v_funny.
   unfold not. intros. unfold normal_form in H. apply H.
   exists (C 0). apply ST_PlusConstConst. Qed.
  (* FILL IN HERE *)
(** [] *)
End Temp1.
...
Lemma value_not_same_as_normal_form :
  exists v, value v /\ ~ normal_form step v.
Proof. exists (C 0). split.
   - apply v_const.
   - unfold normal_form. unfold not. intros. apply H.
     exists (P (C 0) (C 0)). apply ST_Funny. Qed.
  (* FILL IN HERE *) 
...
Proof. exists (P (C 0) (P (C 0) (C 0))). split.
   - unfold not. intros. inversion H.
   - unfold normal_form, not. intros.
     destruct H. inversion H. inversion H3. Qed.
  (* FILL IN HERE *)
(** [] *)
...
Definition bool_step_prop3 :=
   ...
   ==>
     tif
       ...
       (tif ttrue ttrue ttrue)
       tfalse.
Lemma bool_step_prop3_correct: bool_step_prop3.
Proof. unfold bool_step_prop3. eapply ST_If. eapply ST_IfTrue. Qed.
(* FILL IN HERE *)
(** [] *)
...
Proof. intros. induction t.
   ...
   - right. destruct IHt1.
     + destruct t1 eqn:E.
       * exists t2. eapply ST_IfTrue.
       * exists t3. eapply ST_IfFalse.
       * inversion H.
     + destruct H. exists (tif x t2 t3). apply ST_If. assumption. Qed.
  (* FILL IN HERE *)
(** [] *)
...
Theorem step_deterministic :
  deterministic step.
Proof. unfold deterministic. intros. generalize dependent y2.
   induction H.
   - intros. inversion H0.
     + reflexivity.
     + inversion H4.
   ...
   - intros. inversion H0.
     + subst. inversion H.
     + subst. inversion H.
     + subst. rewrite (IHstep t1'0 H5). reflexivity. Qed.
  (* FILL IN HERE *) 
(** [] *)
...
Example bool_step_prop4_holds :
  bool_step_prop4.
Proof. unfold bool_step_prop4. apply ST_ShortCircuit. Qed.
  (* FILL IN HERE *) 
(** [] *)
...
Lemma test_multistep_4:
  ...
  ==>*
      P
        (C 0)
        (C (2 + (0 + 3))).
Proof. eapply multi_step. apply ST_Plus2. apply v_const.
   apply ST_Plus2. apply v_const. apply ST_PlusConstConst.
   simpl. eapply multi_step. apply ST_Plus2. apply v_const.
   apply ST_PlusConstConst. simpl. apply multi_refl. Qed.
  (* FILL IN HERE *)
(** [] *)
...
Proof.
  ...
  - intros. inversion P21. reflexivity. destruct P12. exists y. assumption.
  - intros. inversion P21.
    + subst. destruct P22. exists y. assumption.
    + subst. assert (y=y0). eapply step_deterministic. apply H.
      apply H0. subst. apply IHP11. assumption. assumption. assumption. Qed.
  (* FILL IN HERE *)
(** [] *)
...
Proof. intros. induction H. apply multi_refl.
   eapply multi_trans. assert (P t1 t2==>* P (C n1) t2).
   apply multistep_congr_1. assumption. apply H1.
   assert (P (C n1) t2==>*P (C n1) (C n2)). apply multistep_congr_2.
   apply v_const. assumption. eapply multi_trans. apply H1.
   eapply multi_step. eapply ST_PlusConstConst. apply multi_refl. Qed.
  (* FILL IN HERE *) 
(** [] *)
...
Proof.
  ...
  - intros. inversion H. subst. apply E_Plus. apply E_Const.
    apply E_Const.
  - intros. inversion H. subst. eapply E_Plus. apply IHHs. assumption.
    assumption.
  - intros. inversion H0. subst. eapply E_Plus. assumption. apply IHHs. assumption. Qed.
  (* FILL IN HERE *) 
(** [] *)
...
Proof. unfold normal_form_of. unfold step_normal_form.
   intros t t' H. destruct H. induction H.
   - assert (value x). apply nf_is_value. assumption. inversion H.
     exists n. split. reflexivity. apply E_Const.
   - destruct IHmulti. assumption. exists x0. split. destruct H2. assumption.
     destruct H2. SearchAbout [eval step]. eapply step__eval. apply H. apply H3. Qed.
(* FILL IN HERE *)
(** [] *)
...
Proof. intros t. induction t.
   - intros. simpl. split. intros. rewrite H. apply E_Const.
     intros. inversion H. reflexivity.
   - intros. simpl. split. intros. rewrite <-H.
     assert (t1\\(evalF t1)). apply IHt1. reflexivity.
     assert (t2\\(evalF t2)). apply IHt2. reflexivity.
     eapply E_Plus. assumption. assumption.
     intros. inversion H. subst. destruct (IHt1 n1). rewrite (H1 H2).
     destruct (IHt2 n2). rewrite (H5 H4). reflexivity. Qed.
(* FILL IN HERE *)
(** [] *)
...
Proof. intros. destruct H. unfold par_loop.
   unfold par_loop. eapply multi_step.
   apply CS_Par2. eapply CS_While. eapply multi_step.
   apply CS_Par2. apply CS_IfStep. eapply BS_Eq1. apply AS_Id.
   eapply multi_step. apply CS_Par2. apply CS_IfStep.
   apply BS_Eq. rewrite H0. simpl. eapply multi_step. eapply CS_Par2.
   apply CS_IfTrue. eapply multi_step. apply CS_Par2. 
   eapply CS_SeqStep. apply CS_AssStep. apply AS_Plus1. apply AS_Id. eapply multi_step. apply CS_Par2. apply CS_SeqStep.
   eapply CS_AssStep. apply AS_Plus. rewrite H. eapply multi_step.
   apply CS_Par2. apply CS_SeqStep. apply CS_Ass.
   eapply multi_step. apply CS_Par2. apply CS_SeqFinish.
   SearchAbout [S plus]. rewrite (NPeano.Nat.add_1_r n).
   apply multi_refl. Qed.
(* FILL IN HERE *)
(** [] *)
...
Proof. unfold par_loop. intros n. induction n.
   - intros. exists st. split. apply multi_refl. assumption.
   - intros. destruct (IHn st H). exists (t_update x X (n+1)). split. 
     clear IHn. destruct H0. eapply multi_trans. apply H0.
     eapply multi_step. apply CS_Par2. apply CS_While.
     eapply multi_step. apply CS_Par2. apply CS_IfStep.
     apply BS_Eq1. apply AS_Id. eapply multi_step.
     apply CS_Par2. apply CS_IfStep. apply BS_Eq.
     destruct H1. rewrite H2. simpl. eapply multi_step.
     apply CS_Par2. apply CS_IfTrue. eapply multi_step.
     apply CS_Par2. apply CS_SeqStep. apply CS_AssStep.
     apply AS_Plus1. apply AS_Id. eapply multi_step.
     apply CS_Par2. apply CS_SeqStep. apply CS_AssStep.
     apply AS_Plus. eapply multi_step. apply CS_Par2.
     apply CS_SeqStep. rewrite H1. apply CS_Ass. eapply multi_step.
     apply CS_Par2. apply CS_SeqFinish. apply multi_refl.
     split. unfold t_update. simpl. apply NPeano.Nat.add_1_r.
     unfold t_update. simpl. destruct H0. destruct H1. assumption. Qed.
  (* FILL IN HERE *)
(** [] *)
