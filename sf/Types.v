...
Example some_term_is_stuck :
  exists t, stuck t.
Proof. exists (tiszero ttrue). unfold stuck. split.
   - unfold normal_form, not. intros. destruct H. inversion H.
     inversion H1.
...
Proof. intros t. unfold normal_form, not. intros Ht. induction Ht.
   - intros. destruct H0. inversion H.
     + subst. inversion H0.
     + subst. inversion H0.
   - induction H.
     + intros. destruct H. inversion H.
     + intros. destruct H0. inversion H0. apply IHnvalue.
...
Proof. unfold deterministic. intros. generalize dependent y2.
   induction H; intros.
   - inversion H0. reflexivity. inversion H4.
   - inversion H0. reflexivity. inversion H4.
   - inversion H0. subst. inversion H. subst. inversion H. subst.
     rewrite (IHstep t1'0 H5). reflexivity.
   - inversion H0. subst. erewrite (IHstep). reflexivity. apply H2.
   - inversion H0. reflexivity. subst. inversion H1.
   - assert (step_normal_form t1). apply value_is_nf. right. assumption.
     inversion H0. reflexivity. subst. inversion H3. subst. destruct H1.
     exists t1'0. assumption.
   - inversion H0. subst. inversion H. subst.
     assert (step_normal_form y2). apply value_is_nf. right. assumption.
     inversion H. subst. destruct H1. exists t1'0. assumption. subst.
     erewrite (IHstep t1'0 H2). reflexivity.
   - inversion H0. reflexivity. inversion H1.
   - inversion H0. reflexivity. subst. assert (step_normal_form t1).
     apply value_is_nf. right. assumption. inversion H2. subst.
     destruct H1. exists t1'0. assumption.
   - inversion H0. subst. inversion H. subst. assert (step_normal_form t0). apply value_is_nf. right. assumption. inversion H. subst. destruct H1. exists t1'0. assumption. subst. rewrite (IHstep t1'0 H2). reflexivity. Qed.
  (* FILL IN HERE *)
(** [] *)
