...
Inductive substi (s:tm) (x:id) : tm -> tm -> Prop :=
  | s_var1 :
      substi s x (tvar x) s
(* FILL IN HERE *)
  |s_var2:forall x',beq_id x x'=false-> substi s x (tvar x') (tvar x')
  |s_tabs1: forall t1 T,substi s x (tabs x T t1) (tabs x T t1)
  |s_tabs2:forall x' t1 t1' T,beq_id x x'=false->
                   substi s x t1 t1'->
                   substi s x (tabs x' T t1)  (tabs x' T  t1')
  |s_app:forall t1 t1' t2 t2',substi s x t1 t1'->
                         substi s x t2 t2'->
                         substi s x (tapp t1 t2) (tapp t1' t2')
  |s_true: substi s x ttrue ttrue
  |s_false:substi s x tfalse tfalse
  |s_tif:forall t1 t2 t3 t1' t2' t3',
...
Proof. split. generalize dependent t'.
   - induction t.
     + intros. destruct (beq_id x0 i) eqn:E.
       * unfold subst in H. rewrite E in H. assert (x0=i).
         apply beq_id_true_iff. assumption. rewrite <-H0. subst.
         apply s_var1.
       * unfold subst in H. rewrite E in H. subst. eapply s_var2. assumption.
     + intros. rewrite <-H; clear H. simpl.  eapply s_app.
       apply IHt1. reflexivity. apply IHt2. reflexivity.
     + intros. destruct (beq_id x0 i) eqn:E.
       * assert (x0=i). apply beq_id_true_iff. assumption. rewrite <-H0.
         subst. simpl. rewrite E. apply s_tabs1.
       * rewrite <-H. simpl. eapply s_tabs2. assumption. rewrite E.
         apply IHt. reflexivity.
     + simpl. intros. rewrite <-H. apply s_true.
     + simpl. intros. rewrite <-H. apply s_false.
     + simpl. intros. rewrite <-H. apply s_tif. apply IHt1. reflexivity.
       apply IHt2. reflexivity. apply IHt3. reflexivity.
   - intros. induction H.
     + simpl. erewrite <-beq_id_refl. reflexivity.
     + simpl. rewrite H. reflexivity.
     + simpl. erewrite <-beq_id_refl. reflexivity.
     + simpl. rewrite H. rewrite IHsubsti. reflexivity.
     + simpl. rewrite IHsubsti1, IHsubsti2. reflexivity.
     + reflexivity.
     + reflexivity.
     + simpl. rewrite IHsubsti1, IHsubsti2, IHsubsti3. reflexivity. Qed.
(* FILL IN HERE *)
(** [] *)
  ...
  empty |-
    (tabs x TBool
       (tabs y (TArrow TBool TBool)
          (tapp (tvar y) (tapp (tvar y) (tvar x))))) \in
    (TArrow TBool (TArrow (TArrow TBool TBool) TBool)).
Proof. apply T_Abs. apply T_Abs. eapply T_App. apply T_Var. apply update_eq. eapply T_App. apply T_Var. apply update_eq. apply T_Var. unfold update. unfold t_update.
...
Proof with auto using update_eq.
  ...
  eapply T_App. apply T_Var. unfold update, t_update. simpl. reflexivity. apply T_Var. unfold update, t_update. simpl. reflexivity. Qed.
(* FILL IN HERE *)
(** [] *)
...
(** Another nonexample:
    ...
    ~ (exists S, exists T,
          empty |- \x:S. x x \in T).
*)
Lemma TArrow_T1_T2_not_T1:forall T1 T2,~(TArrow T1 T2=T1).
                            Proof. intros T1. induction T1. intros. unfold not.
                               intros. inversion H. intros. unfold not in *.
                               intros. inversion H. eapply IHT1_1. apply H1. Qed.
Example typing_nonexample_3 :
  ~ (exists S, exists T,
        empty |-
          ...
          T).
Proof. unfold not. intros. destruct H as [S]. destruct H as [T].
   generalize dependent S. induction T.
   - intros. inversion H.
   - intros. inversion H. subst. inversion H2. subst.
     inversion H4. subst. inversion H6. subst. rewrite H3 in H5. inversion H5. simpl. eapply TArrow_T1_T2_not_T1. apply H1. Qed.
  (* FILL IN HERE *)
(** [] *)
