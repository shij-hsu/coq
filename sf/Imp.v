...
Proof. intros. induction b; try (reflexivity); try (simpl); try( repeat rewrite optimize_0plus_sound; reflexivity);
             try (rewrite IHb; reflexivity); try(rewrite IHb1,IHb2; reflexivity). Qed.
  (* FILL IN HERE *)
(** [] *)
...
Inductive ceval : com -> state -> status -> state -> Prop :=
  ...
  | E_Seq_C : forall c1 c2 st st' st'' sta,
      c1 / st  \\SContinue/ st' ->
      c2 / st' \\sta/ st'' ->
      (c1 ;; c2) / st \\sta/ st''
...
Proof. intros. destruct s.
   ...
   - inversion H. inversion H2. inversion H3. reflexivity. Qed.
  (* FILL IN HERE *)
...
Theorem while_stops_on_break : forall b c st st',
  ...
  c / st \\ SBreak / st' ->
  (WHILE b DO c END) / st \\ SContinue / st'.
Proof. intros. apply (E_WhileLoop_B st st'). apply H. apply H0. Qed.
  (* FILL IN HERE *) 
