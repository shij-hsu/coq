...
*)
(** [] *)
Lemma My_b_a_b_a:forall a b,leb a b=true->b=a+(b-a).
Proof. induction a. simpl. intros. omega.
   intros. destruct b. inversion H. simpl in *.
   pattern b at 1. rewrite (IHa b H). reflexivity. Qed.
Lemma if_minus_plus_reloader:
  {{fun st=>True}}(IFB BLe (AId X) (AId Y) THEN Z::=AMinus (AId Y) (AId X) ELSE Y::=APlus (AId X) (AId Z) FI){{fun st=>(st Y)=(st X)+(st Z)}}.
Proof. eapply hoare_if. unfold bassn in *.
   eapply hoare_consequence_pre. eapply hoare_asgn.
   simpl. unfold assert_implies, assn_sub, t_update.
   intros. simpl. apply My_b_a_b_a. apply H.
   unfold bassn. eapply hoare_consequence_pre.
   apply hoare_asgn. unfold assert_implies, assn_sub, t_update. simpl. intros. reflexivity. Qed.
(* ================================================================= *)
...
Proof. intros. assert ((fun st=>st X = m)->> fun st=>parity(st X)=parity(m)).
   { unfold assert_implies. intros. rewrite H. reflexivity. }
   eapply hoare_consequence_pre with (fun st : state => parity (st X) = parity m).
   eapply hoare_consequence_post with (fun st=>parity (st X) = parity m /\ ~ bassn (BLe (ANum 2) (AId X)) st).
   apply hoare_while. unfold bassn, hoare_triple.
   intros. inversion H0. subst. simpl. destruct H1.
   rewrite <-H1. eapply parity_ge_2.
   apply leb_iff. unfold beval in H2. apply H2.
   unfold bassn, assert_implies. intros. destruct H0.
   rewrite <-H0. symmetry. eapply parity_lt_2.
   unfold not in *. intros. apply H1. unfold beval.
   apply leb_iff. simpl. assumption. assumption. Qed.
  (* FILL IN HERE *) 
(** [] *)
...
Proof. unfold is_wp, hoare_triple, assert_implies. split.
   - intros. inversion H. subst. simpl.
     unfold t_update. simpl. SearchAbout "add".
     eapply NPeano.Nat.add_le_mono_r in H0.
     apply H0.
   - intros. eapply NPeano.Nat.add_le_mono_r.
     assert (st Y+1<=4+1).
     { simpl. remember (t_update st X (st Y+1)) as st''.
       assert (st'' X<=5). eapply (H st).
       rewrite Heqst''. apply E_Ass. simpl. reflexivity.
       apply H0. rewrite Heqst'' in H1. unfold t_update in H1. simpl in H1. assumption. }
     eassumption. Qed.
  (* FILL IN HERE *)
(** [] *)
...
Theorem hoare_asgn_weakest : forall Q X a,
  is_wp (Q [X |-> a]) (X ::= a) Q.
Proof. unfold is_wp. intros. split.
   - apply hoare_asgn.
   - intros. unfold hoare_triple, assert_implies, t_update, assn_sub in *. intros. eapply H.
     apply E_Ass. reflexivity. apply H0. Qed.
(* FILL IN HERE *)
(** [] *)
...
Lemma hoare_havoc_weakest : forall (P Q : Assertion) (X : id),
  {{ P }} HAVOC X {{ Q }} ->
  P ->> havoc_pre X Q.
Proof. intros. unfold havoc_pre, assert_implies, hoare_triple in *. intros. eapply H. apply E_Havoc. apply H0. Qed.
(* FILL IN HERE *) 
End Himp2.
(** [] *)
...
Example slow_assignment_dec (m:nat) : dcom :=
  ({{fun st=>st X=m}}->>{{fun st=>st X+0=m}}Y::=(ANum 0)
    {{fun st=>st X+st Y=m}};;
...
Proof. (* FILL IN HERE *) intros. verify. Qed.
(** [] *)
...
Definition factorial_dec (n:nat):dcom :=
  (
    {{fun st=>True}}->>{{fun st=>1=1}}
    X::=(ANum 1)
    {{fun st=>st X=1}}->>{{fun st=>st X=1/\ 1=1}};;
    Y::=(ANum 1){{fun st=>st X=1 /\ st Y=1}}
    ->>{{fun st=>st Y=real_fact (st X-1)/\ st X>=1}};;
    WHILE BNot (BEq (AId X) (ANum (n+1))) DO
    {{fun st=>st Y=real_fact (st X-1) /\st X>=1/\ ~(st X=n+1)}}
    ->>{{fun st=>((st Y*st X)=real_fact (st X))/\ st X>=1}}
    Y::=AMult (AId Y) (AId X)
    {{fun st=>st Y=real_fact (st X)/\st X>=1 }};;
...
Lemma factorial_dec_correct: forall m,
    dec_correct (factorial_dec m).
Proof. intros m. verify.
   - destruct (st X).
    + inversion H0.
    + simpl. SearchAbout "minus". rewrite <-(minus_n_O n).
     rewrite plus_comm. pattern (n * real_fact n) at 1.
     rewrite mult_comm.
     eapply (mult_succ_r (real_fact n)).
   - rewrite plus_comm. SearchAbout "minus".
     rewrite (minus_plus 1 (st X)). reflexivity.
   - rewrite plus_comm. rewrite (minus_plus 1 m). reflexivity. Qed.
(** [] *)
