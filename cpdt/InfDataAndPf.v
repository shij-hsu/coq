...
Definition t1 A (s:stream A) : stream A :=
match s with
| Cons _ s' => s'
end.
...
Theorem ones_eq : ones = ones'. Abort.
...
Section stream_eq_coind.
  ...
  Theorem stream_eq_conid : forall s1 s2, R s1 s2 -> stream_eq s1 s2.
    cofix; destruct s1; destruct s2; intro.
    generalize (Cons_case_hd H); intros Heq; simpl in Heq; rewrite Heq.
    constructor. apply stream_eq_conid. apply (Cons_case_tl H). Qed.
End stream_eq_coind.

Print stream_eq_conid.

Theorem ones_eq' : stream_eq ones ones'.
  apply (stream_eq_conid (fun s1 s2 => s1 = ones /\ s2 = ones')); crush. Qed.

Section strream_eq_loop.
  Variable A : Type.
 ...
 (*   apply stream_eq_conid with ( fun s1' s2' => s1' = s1 /\ s2' = s2).
    - intros s0 s3 H; destruct H as [H1 H2];rewrite H1;rewrite H2; try split; assumption.
    - intros s0 s3 H; destruct H as [H1 H2];rewrite H1;rewrite H2. split; assumption.
    - split; reflexivity.*)
    apply (stream_eq_conid (fun s1' s2'=> s1'=s1 /\ s2'=s2)); crush. Qed.
End strream_eq_loop.

Theorem ones_eq''': stream_eq ones ones'.
  apply stream_eq_loop; crush. Qed.
...
(*
Eval simpl in approx fact_slow 30.
Eval simpl in approx fact_iter 30.
 *)

Lemma fact_def : forall x n, fact_iter' x (fact n*S n) =fact_iter' x (fact (S n)).
  simpl; intros; f_equal; ring. Qed.

Hint Resolve fact_def.

Lemma fact_eq' : forall n, stream_eq (fact_iter' (S n) (fact n)) (fact_slow' n).
  intros; apply (stream_eq_conid (fun s1 s2=> exists n, s1=fact_iter' (S n) (fact n) /\ s2=fact_slow' n)); crush; eauto. Qed.
...
Section stream_eq_onequant.
  ...
  Theorem stream_eq_onequant : forall x, stream_eq (f x) (g x).
    intros; apply (stream_eq_conid (fun s1 s2 => exists x, s1=f x /\ s2=g x));
      crush; eauto. Qed.
End stream_eq_onequant.

Lemma fact_eq'': forall n, stream_eq (fact_iter' (S n) (fact n)) (fact_slow' n).
  apply stream_eq_onequant; crush; eauto. Qed.

(* 5.3 *)
Definition var := nat.
Definition vars := var->nat.
Definition set (vs:vars) (v:var) (n:nat) :vars := fun v'=> if beq_nat v v' then n else vs v'.
...
Section evalCmd_coind.
  ...
  Theorem evalCmd_coind : forall vs1 c vs2,R vs1 c vs2 -> evalCmd vs1 c vs2.
    ...
    - destruct (SeqCase H) as [? [H1 H2]]; econstructor; eauto.
    - destruct (WhileCase H) as [[? ?]|[? [? [? ?]]]]; subst; econstructor; eauto. Qed.
End evalCmd_coind.
...
Lemma optCmd_correct1 : forall vs1 c vs2, evalCmd vs1 c vs2 ->
  ...
  Theorem optCmd_correct : forall vs1 c vs2, evalCmd vs1 (optCmd c) vs2 <-> evalCmd vs1 c vs2.
    intuition; apply optCmd_correct1 || apply optCmd_correct2; assumption. Qed.
